...
class Tree(object):
    def __init__(self, leaf_count, width, height, max_distance):
        self.leaves = []
        self.branches = []
        
        for i in range(leaf_count):
            self.leaves.append(Leaf(PVector(random(width), random(height))))
            
        # root branch is pointing directly up
        root_direction = PVector(0, -1)
        root_position = PVector(width / 2, height / 2)
